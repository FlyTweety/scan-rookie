    ...
    def scan(self, target_ip_list): #只传ip不传端口，因为默认是5353
        ...
        for target_ip in target_ip_list:
            ...
            try:
                ...
                print("[DNS-SD Scanning] %s OFFLINE" % target_ip)
                utils.log("[DNS-SD Scanning] %s OFFLINE" % target_ip)
                self.known_mdns_info_list.append({"ip":target_ip, "scan_time":time.time(), "status":False, "services":[]})
                return
            resp = DNS(data)
            #resp.show()

            print("[DNS-SD Scanning] %s ONLINE" % target_ip)
            utils.log("[DNS-SD Scanning] %s ONLINE" % target_ip)
